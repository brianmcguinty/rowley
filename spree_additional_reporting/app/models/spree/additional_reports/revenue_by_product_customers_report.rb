module Spree::AdditionalReports
  class RevenueByProductCustomersReport < IntervalReport
...
from (
	select p.id product_id, 
		   ...
		   sum(si.items_qty) total_qty, 
		   sum(si.items_qty * si.item_total_price) total_amount, 
		   mv.sku product_sku,
		   si.on_date,
		   si.customer_name,
		   (@n := @n + 1) cnt,
	...
	from (
		select 
            ...
            + round(order_misc_adjustments_total * ovi.item_price / item_total, 2) item_total_price,
			o.on_date,
			o.customer_name
		from (
         ...
         sum(if (a.originator_type != 'Spree::LensMetaPrescription ', a.amount, 0)) order_misc_adjustments_total,
			   date(o.completed_at) on_date,
			   concat(ba.firstname, ' ', ba.lastname) customer_name,
         o.item_total
			from spree_orders o,
				   spree_adjustments a,
				   spree_addresses ba
	...
	where si.variant_id = v.id 
		  ...
		  and mv.is_master = 1
		  and v.is_master = 0
	group by v.id, si.on_date, si.customer_name
) r
...
group by r.variant_id, r.on_date, r.customer_name
order by r.product_name, r.variant_id, r.on_date desc, r.customer_name
      "
      flat_data = ActiveRecord::Base.connection.select_all(query)
      self.class.group_rows(flat_data, ['variant_id', 'cnt'], Proc.new { |t, r| { 
